...
export default function EmployeeInstructionsPage() {
  ...
  // Derive Host URL for media asset serving
  ...
  useEffect(() => {
    ...
    if (selectedInst && isFullscreen) {
      if (acknowledgedIds.includes(selectedInst.id) || selectedInst.acknowledged) {
        setCountdown(0);
        setTimerActive(false);
      } else {
        setCountdown(10); // 10 seconds forced reading requirement
        setTimerActive(true);
  ...
  const fetchInstructions = async () => {
    setIsLoading(true);
    try {
      // Fetch instructions that apply to the employee
      const res = await authFetch(`${apiUrl}/instructions`);
      if (res.ok) {
        const data = await res.json();
        // If employee is in a specific department, filter or prioritize their department instructions
        const filtered = user?.department?.id
          ? data.filter((i: any) => !i.departmentId || i.departmentId === user?.department?.id)
          : data;
        
        setInstructions(filtered);
        
        // Extract already signed off instruction IDs
        const acked = filtered.filter((i: any) => i.acknowledged).map((i: any) => i.id);
        setAcknowledgedIds(acked);

        if (filtered.length > 0) {
          setSelectedInst(filtered[0]);
        }
      } else {
    ...
    try {
      const res = await authFetch(`${apiUrl}/acknowledgements`, {
        ...
        body: JSON.stringify({
          ...
          machineId: selectedInst.machineId || 'generic-machine-id',
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-white font-sans">Assigned SOP Instructions</h1>
        <p className="text-sm text-slate-400">
          Review machine checklists, safety regulations, and operating standards assigned to you
        </p>
      </div>
      ...
      <div className="relative glass-panel p-4 border-none shadow-sm flex items-center gap-2">
        ...
        <input
          type="text"
          placeholder="Filter instructions by title or content keywords..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="flex-1 bg-transparent text-xs text-white focus:outline-none placeholder:text-slate-650"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <h3 className="text-xxs font-bold text-slate-400 uppercase tracking-wider mb-1 px-1">
            Instruction Directory ({filteredInstructions.length})
          </h3>
          {isLoading ? (
            ...
            <div className="glass-panel p-6 text-center border-none">
              <p className="text-xxs text-slate-400">No active instructions found.</p>
            </div>
          ) : (
            <div className="space-y-2 max-h-[70vh] overflow-y-auto pr-1">
              {filteredInstructions.map((inst) => {
                const isSelected = selectedInst?.id === inst.id;
                const isAcked = acknowledgedIds.includes(inst.id) || inst.acknowledged;
                return (
                  <button
                    key={inst.id}
          ...
          {selectedInst ? (
            <div className="glass-panel p-6 border-none space-y-6">
              ...
              <div className="flex justify-between items-start flex-wrap gap-2 pb-4 border-b border-white/10">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xxs px-2 py-0.5 bg-white/5 border border-white/10 rounded font-semibold text-slate-300">
              ...
              {/* Attachments (SOP Manuals & Multimedia Attachments) */}
              {selectedInst.attachments && selectedInst.attachments.length > 0 && (
                <div className="space-y-2 pt-4 border-t border-white/10">
                  <h4 className="text-xxs font-bold text-slate-400 uppercase tracking-wider">
                    SOP Manuals & Multimedia Attachments
                  </h4>
                  <div className="flex flex-col gap-2">
                    {selectedInst.attachments.map((att: any) => (
                      <button
                        key={att.id}
                        onClick={() => handleDownloadAttachment(att.id, att.filename)}
                        disabled={isDownloading === att.id}
                        className="flex items-center justify-between p-2.5 rounded-lg bg-white/5 hover:bg-white/10 text-xxs font-semibold border border-white/10 transition-colors w-full text-left disabled:opacity-50 cursor-pointer animate-none"
                      >
                        <div className="flex items-center gap-2">
                        ...
                        <span className="text-cyan-400 hover:underline">
                          {isDownloading === att.id ? 'Downloading...' : 'Download'}
                        </span>
                      </button>
                    ))}
                  </div>
              ...
              <div className="pt-6 border-t border-white/10 flex flex-col items-center gap-4 text-center">
                {acknowledgedIds.includes(selectedInst.id) || selectedInst.acknowledged ? (
            ...
            <div className="glass-panel p-16 text-center border-none">
              <p className="text-xxs text-slate-400">Select an instruction from the left menu to review details.</p>
            </div>
          )}
        </div>
      </div>
